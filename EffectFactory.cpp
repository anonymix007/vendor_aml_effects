...
Factory::Factory(const std::string& file, std::shared_ptr<IFactory> impl) : mConfig(EffectConfig(file)), mImpl(impl) {
...
ndk::ScopedAStatus Factory::getDescriptorWithUuid_l(const AudioUuid& uuid, Descriptor* desc) {
    ...
    if (mEffectLibMap.count(uuid)) {
        ...
        LOG(ERROR) << __func__ << ": remote impl effect is not supported here: " << ::android::audio::utils::toString(uuid);
    }
...
ndk::ScopedAStatus Factory::queryEffects(const std::optional<AudioUuid>& in_type_uuid,
    ...
    for (const auto& id : idList) {
        if (mEffectLibMap.count(id.uuid)) {
            ...
            // Assume remote impl effect
            std::vector<Descriptor> implDescriptors;
            auto result = mImpl->queryEffects(id.type, id.uuid, id.proxy, &implDescriptors);
            if (!result.isOk()) {
                return result;
            }
            std::copy(implDescriptors.begin(), implDescriptors.end(), std::back_inserter(*_aidl_return));
        }
    }
    return ndk::ScopedAStatus::ok();
}
...
ndk::ScopedAStatus Factory::queryProcessing(const std::optional<Processing::Type>& in_type,
    ...
    std::vector<Processing> implProcessings;
    auto result = mImpl->queryProcessing(in_type, &implProcessings);
    if (!result.isOk()) {
        return result;
    }

    std::copy(implProcessings.begin(), implProcessings.end(), std::back_inserter(*_aidl_return));
    return ndk::ScopedAStatus::ok();
}
...
ndk::ScopedAStatus Factory::createEffect(const AudioUuid& in_impl_uuid,
    ...
    if (mEffectLibMap.count(in_impl_uuid)) {
        ...
        *_aidl_return = effectSp;
        ...
        // Assume remote impl effect
        LOG(DEBUG) << __func__ << ": creating remote impl effect: " << ::android::audio::utils::toString(in_impl_uuid);
        return mImpl->createEffect(in_impl_uuid, _aidl_return);
    }
...
ndk::ScopedAStatus Factory::destroyEffectImpl_l(const std::shared_ptr<IEffect>& in_handle) {
    ...
    if (auto effectIt = mEffectMap.find(wpHandle); effectIt != mEffectMap.end()) {
        ...
        // Assume remote impl effect
        LOG(ERROR) << __func__ << ": destroying remote impl effect: " << in_handle;
        return mImpl->destroyEffect(in_handle);
    }
}
...
void Factory::queryImplEffects() NO_THREAD_SAFETY_ANALYSIS {
    std::vector<Descriptor> implEffects;
    auto status = mImpl->queryEffects(std::nullopt, std::nullopt, std::nullopt, &implEffects);
    CHECK(status.isOk());
    for (const auto& implDesc: implEffects) {
        mIdentitySet.insert(implDesc.common.id);
    }
}
